                  ...
                  <div key={`${key}-${index}`} className="flip-card" ref={(el) => (cardRefs.current[index] = el)}>
                    <div className="flip-card-inner">
                      ...
                      <div className="flip-card-back aspect-[3/2]">
                        <ul className="list-disc list-outside pl-4 space-y-2 text-black text-sm lg:text-base">
                          {course.description.map((item, idx) => (
...
.flip-card-back {
    ...
    transform: rotateY(180deg);
    padding: 0; /* Remove padding to allow ul to be positioned freely */
    color: #003073 !important; /* Ensure text color is set */
...
.flip-card-back ul {
    margin: -20px 0 0 0; /* Pull it outside the top */
