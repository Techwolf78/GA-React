...
const BrochureSection = () => {
  ...
  const [isFormSubmitted, setIsFormSubmitted] = useState(false); // To track if form is already submitted
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Set the form as submitted so the button remains disabled permanently
      ...
      // Re-enable the button and reset the state after submission attempt
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg relative w-full md:w-1/3">
            ...
            <form id="contactForm" onSubmit={handleSubmit}>
              ...
              <button
                type="submit"
                disabled={isSubmitting || isFormSubmitted} // Disable button permanently after submission
                className={`bg-yellow-400 text-gray-800 py-2 rounded-lg font-bold transition-colors w-full ${isSubmitting || isFormSubmitted ? "bg-gray-400 cursor-not-allowed" : "hover:bg-yellow-300"}`}
              >
                {isSubmitting ? "Submitting..." : isFormSubmitted ? "Already Submitted" : "Send Details"} {/* Change button text */}
              </button>
            </form>
          </div>
        </div>
